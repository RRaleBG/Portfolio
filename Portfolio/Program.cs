...
var connStr = builder.Configuration.GetConnectionString("Default") ?? $"Data Source={Path.Combine(dataDir, "app.db")}";
...
builder.Services.AddIdentity<IdentityUser, IdentityRole>(options =>
{
    ...
    options.SignIn.RequireConfirmedEmail = true; // enforce confirmation
})
...
// Admin-only: recreate the SQLite database from current EF models (development tool)
app.MapPost("/api/admin/recreatedb", async (IServiceProvider services) =>
{
    ...
    try
    {
        ...
        {
            logger.LogWarning(ex, "Failed to delete DB file before recreate");
        }

        // Ensure DB created according to current models
        ...
        if (adminUser == null)
        {
            ...
            var createResult = await userMgr.CreateAsync(adminUser, "Admin123!");
        ...
        if (!await db.Projects.AnyAsync())
        {
            db.Projects.AddRange(
                new Project { Title = "AI Portfolio", Summary = "GPT-powered portfolio chat.", Url = "https://cloud.rajcicrados.rs", Tags = "AI,GPT,.NET", Date = DateTime.UtcNow.AddMonths(-1) },
                new Project { Title = "Razor Pages Site", Summary = "Clean, fast UI.", Url = "https://coolify.rajcicrados.rs", Tags = "Razor,ASP.NET Core", Date = DateTime.UtcNow.AddMonths(-2) }
            );
            await db.SaveChangesAsync();
        }

        return Results.Ok(new { ok = true, message = "Database recreated and seeded." });
    }
    catch (Exception ex)
    {
...
// Apply migrations & seed initial admin user if missing
